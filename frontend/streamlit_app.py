...
with st.container(border=True):
    st.subheader("Inputs")
    upload_col, config_col = st.columns(2, gap="large")
    ...
    if dataset is not None and not preview.empty:
...
if result:
    with results_box:
        ...
        findings_tab, timeline_tab, report_tab = st.tabs(["Findings", "Agent Timeline", "Report"])
            ...
            if job_id:
                try:
                    ...
                    if timeline:
                        ...
                        st.info("No timeline entries returned.")
                except Exception as exc:
